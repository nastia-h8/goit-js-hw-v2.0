...
function findNarcissus(arr) {
  for (let i = 0; i < arr.length; i += 1) {
    let isNarcissus = true;
    
    for (let j = 0; j < arr.length; j += 1){

      if (i !== j) {
        const result = arr[i].know.length === 0 && arr[j].know.includes(arr[i].name);
        if (result) { 
        } else {
          isNarcissus = false;
          break;
...
// Після чого відрендерити список фільмів як наведено в прикладі нижче(Expected result),
...
// films.map(film => {
//   const filmGenres = [];
//   const genresIdsList = film.genreIDs;

//   genresInfo.genres.map(genre => {
//     if (!genresIdsList.includes(genre.id)) {
//       return;
//     }
//     filmGenres.push(genre.name);
//   })

//   film.genreNames = filmGenres;
// })
...
//   const markup = arr.map(({ title, genreNames }) => {
//     if (genreNames.length <= 2) {
//       return `<li>${title}: ${genreNames}</li>`
//     } else {
//       const slicesGenreNames = genreNames.slice(0, 2);
//       return `<li>${title}: ${slicesGenreNames}, Others.</li>`
//     }
...
class Student extends User{
  ...
  getCourse() {
    const delta = new Date().getFullYear() - this.year;
    let course = null;
    switch (delta) {
      case 0: course = 'course: 1';
        break;
      case 1: course ='course: 2';
        break;
      case 2: course ='course: 3';
        break;
      case 3: course ='course: 4';
        break;
          
      default: course = `${this.year + delta} year graduate`;
...
// console.log(student.getEntryYear());
